...
public class OpenApiConfig {
    ...
    public OpenAPI wasacOpenApi() {
        ...
        return new OpenAPI()
                ...
                .tags(List.of(
                        new Tag().name("Authentication").description("Signup, login, change/reset password (OTP recovery)"),
                        new Tag().name("Profile").description("Current user profile — all roles"),
                        new Tag().name("Users").description("Create staff, list, role change, activate/deactivate (ADMIN)"),
                        new Tag().name("Customers").description("Admin create (temp password) or self-signup — profile CRUD"),
